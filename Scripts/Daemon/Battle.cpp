...
#include "..\ECS\Components\TextRendererComponent.h"
        ...
        {           
            ...
            if (!atkDone || !defDone) 
            {
                if (atkFirst) 
                {
                    ...
                    {
                        atkTurn.attackUsed->AttackDaemon(*atk, *def, actionsQueue, true);
                    }                    
                    //actionsQueue.push(next);
                    if (!defDone && CanAttack(def, &defTurn) && !CheckTurnEnded()) 
                    {
                        defTurn.attackUsed->AttackDaemon(*def, *atk, actionsQueue, false);
                    }
                }
                else
                {
